...
test.describe('Kataloge - Lernplattformen', () => {
  test('should login, navigate to Lernplattformen, and add data', async ({ page }) => {
    ...
    // Click on Lernplattformen using the correct selector for sidebar menu item
    await page.locator('a.sidebar--menu-item[title="Lernplattformen"]').click();

    // Verify we're on the Lernplattformen page by checking for a specific element
    await page.waitForSelector('text=Lernplattformen', { timeout: 2000 });
    
    // Click the '+' button to add a new Lernplattform
    ...
    await bezeichnungInput.fill('Z Lernplattform Test');
